...
def pre_normalize(adata, copy=True, highly_genes=None, filter_min_counts=True, size_factors=True, normalize_input=True,
                  logtrans_input=True):
...
def load_h5(filename, n_gene, sparsify=False, skip_exprs=False):
    ...
    adata = sc.AnnData(X)
    adata.obs['Group'] = Y
    adata = pre_normalize(adata, copy=True, highly_genes=n_gene, size_factors=True, normalize_input=False,
                          logtrans_input=True)
    ...
    return X, Y, count_X, X_row,size_factor, cluster_number
